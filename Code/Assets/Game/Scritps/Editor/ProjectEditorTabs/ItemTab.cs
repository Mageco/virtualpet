...
public class ItemTab : BaseTab
{
    ...
    public void ShowTab()
    {
        ...
        if (DataHolder.Items().GetDataCount() > 0)
        {
            ...
            EditorGUILayout.Separator();
            DataHolder.Item(selection).category = EditorGUILayout.Popup("Category", DataHolder.Item(selection).category, DataHolder.Categories().GetNameList(true), GUILayout.Width(pw.mWidth * 2));
			EditorGUILayout.Separator();
			...
			if(fold2)
			{
				...
				DataHolder.Item(selection).sellSetter = (ValueSetter)EditorTab.EnumToolbar("Sell in", (int)DataHolder.Item(selection).sellSetter, typeof(ValueSetter));
			...
			if(fold3)
			{
				DataHolder.Item(selection).useable = EditorGUILayout.BeginToggleGroup("Useable", DataHolder.Item(selection).useable);
			...
			if(fold4)
			{
				...
				{
					if (DataHolder.Item (selection).valueChange [i] == null)
						DataHolder.Item (selection).valueChange [i] = new ValueChange ();
