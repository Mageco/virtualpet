    ...
    public void ShowTab()
    {
        ...
        if (DataHolder.Items().GetDataCount() > 0)
        {
			...
			if(fold2)
			{
                ...
                DataHolder.Items().GetItem(selection, temcategory).isOnShop = EditorGUILayout.Toggle("On Shop", DataHolder.Items().GetItem(selection, temcategory).isOnShop, GUILayout.Width(pw.mWidth));
