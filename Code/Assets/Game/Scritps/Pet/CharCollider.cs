...
        }else if(other.tag == "Pet"){
            ...
            pets.Add(p);
            if(p.actionType != ActionType.Hold && p.actionType != ActionType.Toy && character.enviromentType == EnviromentType.Room && character.isMoving && character.actionType != ActionType.Toy)
            {
                character.OnStop();
            }
