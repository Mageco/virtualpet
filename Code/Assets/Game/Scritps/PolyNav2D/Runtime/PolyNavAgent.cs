...
public class PolyNavAgent : MonoBehaviour{
	...
	public AgentType type = AgentType.Char;
	...
	void LateUpdate(){
		...
		velocity = Truncate(velocity, speed);
		//
		...
		rotation = -Mathf.Atan2(movingDirection.x, movingDirection.y) * 180 / Mathf.PI;
		//rotate if must
		if (rotateTransform){
			float newZ = Mathf.MoveTowardsAngle(transform.localEulerAngles.z, rotation, rotateSpeed * Time.deltaTime);
	...
	Vector2 Seek(Vector2 pos){
		...
		Vector2 desiredVelocity= (pos - position).normalized * speed;
	...
	Vector2 Arrive(Vector2 pos){
		...
		if (dist > 0){
			...
			reqSpeed = Mathf.Min(reqSpeed, speed);
	...
	void LookAhead(){
		...
		var currentLookAheadDistance= Mathf.Lerp(0, lookAheadDistance, velocity.magnitude/speed);
...
public enum AgentType {Char,Pet,NPC};
