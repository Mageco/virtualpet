...
public class SkillData : BaseData
{
	...
	public void AddStatusValue(int index)
	{
		for(int i=0; i<skills.Length; i++)
		{
			skills[i].valueChange = ArrayHelper.Add(new ValueChange(), skills[i].valueChange);
		}
	}
	...
	public void RemoveStatusValue(int index)
	{
		for(int i=0; i<skills.Length; i++)
		{
			skills[i].valueChange = ArrayHelper.Remove(index, skills[i].valueChange);

			for(int k=index; k<skills[i].valueChange.Length-1; k++)
			{
				if(skills[i].valueChange[k].status == index)
				{
					skills[i].valueChange[k].status = 0;
				}
				else if(skills[i].valueChange[k].status > index)
				{
					skills[i].valueChange[k].status -= 1;
				}
			}
		}
	}
	...
	public void SetStatusValueType(int index, StatusValueType val)
	{
		if(!StatusValueType.CONSUMABLE.Equals(val))
		{
			for(int i=0; i<skills.Length; i++)
			{
				skills[i].valueChange[index] = new ValueChange();
			}
		}
	}
