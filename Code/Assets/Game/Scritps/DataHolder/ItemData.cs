...
public class ItemData : BaseData
{
	...
	public void AddStatusValue(int index)
	{
		for(int i=0; i<items.Length; i++)
		{
			items[i].valueChange = ArrayHelper.Add(new ValueChange(), items[i].valueChange);
		}
	}
	...
	public void RemoveStatusValue(int index)
	{
		for(int i=0; i<items.Length; i++)
		{
			for(int j=index; j<items[i].valueChange.Length-1; j++)
			{
				items[i].valueChange[j] = items[i].valueChange[j+1];
			}
			items[i].valueChange = ArrayHelper.Remove(items[i].valueChange.Length-1, items[i].valueChange);

			for(int j=index; j<items[i].valueChange.Length-1; j++)
			{
				items[i].valueChange[j].status = this.CheckForIndex(index, items[i].valueChange[j].status);
			}
		}
	}
