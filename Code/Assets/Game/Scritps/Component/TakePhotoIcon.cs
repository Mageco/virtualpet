...
public class TakePhotoIcon : MonoBehaviour
{
	...
	float size;
	float scaleX;
	public PetPhotoIcon petPhotoIcon;
	...
	IEnumerator TakePhotoCoroutine()
	{
        ...
        for(int i = 0; i < petPhotoIcon.petAccessories.Count; i++)
        {
			if (petPhotoIcon.NextPet())
			{
				...
				yield return new WaitForEndOfFrame();
				var texture = new Texture2D(resWidth, resHeight, TextureFormat.RGBA32, false);
				...
				string path = Application.dataPath + "/Game/Resources/Icons/Accessory/";
				System.IO.File.WriteAllBytes(path + petPhotoIcon.petAccessories[i].name + ".png", _bytes);
				Debug.Log("Complete Take photo " + petPhotoIcon.petAccessories[i].name);
			}
			yield return new WaitForEndOfFrame();
		}
