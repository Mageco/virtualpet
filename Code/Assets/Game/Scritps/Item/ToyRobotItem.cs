...
public class ToyRobotItem : MonoBehaviour
{
    ...
    IEnumerator Hold()
    {
        int soundId = MageManager.instance.PlaySoundName("Toy_Robot_Dance", true);
        if (happyTarget != null)
        {
            happyTarget.OnPick();
    ...
    HappyItem GetDirtyItem()
    {
        if (happyTarget == null)
            happyTarget = FindObjectOfType<HappyItem>();
        return happyTarget;
    }
