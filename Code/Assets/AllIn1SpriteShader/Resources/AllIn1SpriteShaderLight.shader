		...
		SubShader
		{
			...
			Pass
			{
				...
				fixed4 frag(v2f i) : SV_Target
				{
					...
					#if SHAKEUV_ON
					fixed xShake = sin(_Time * _ShakeUvSpeed * 500 * _ShakeUvX);
					fixed yShake = sin(_Time * _ShakeUvSpeed * 500 * _ShakeUvY);
					...
					#if DISTORT_ON
					fixed2 tempDistUv = i.uv;
					tempDistUv.x += (_Time * _DistortTexXSpeed) % 1;
					tempDistUv.y += (_Time * _DistortTexYSpeed) % 1;
					#if ATLAS_ON
					tempDistUv = fixed2((tempDistUv.x - _MinXUV) / (_MaxXUV - _MinXUV), (tempDistUv.y - _MinYUV) / (_MaxYUV - _MinYUV));
					#endif
					fixed distortAmnt = tex2D(_DistortTex, tempDistUv).r * 0.1 * _DistortAmount;
					i.uv.x += distortAmnt;
					i.uv.y += distortAmnt;
					#endif
					...
					uvGlitch.y -= 0.5;
					fixed lineNoise = pow(rand2(floor(uvGlitch * fixed2(24., 19.))*4.0), 3.0) * _GlitchAmount
						* pow(rand2(floor(uvGlitch * fixed2(38., 14.))*4.0), 3.0);
					col = tex2D(_MainTex, i.uv + fixed2(lineNoise * 0.02 * rand2(fixed2(2.0, 1)), 0));
					#endif

					#if CHROMABERR_ON
					fixed4 r = tex2D(_MainTex, i.uv + fixed2(_ChromAberrAmount / 10, 0)) * i.color * _Color;
					fixed4 b = tex2D(_MainTex, i.uv + fixed2(-_ChromAberrAmount / 10, 0)) * i.color * _Color;
					col = fixed4(r.r * r.a, col.g, b.b * b.a, max(max(r.a, b.a) * _ChromAberrAlpha, col.a));
					#endif

					#if FADE_ON
					fixed2 tiledUvFade1 = i.uv;
					fixed2 tiledUvFade2 = i.uv;
					#if ATLAS_ON
					...
					#if BLUR_ON
					#if !BLURISHD_ON
					col = BlurHD(i.uv, _MainTex, _BlurIntensity);
					#else
					col = Blur(i.uv, _MainTex, _BlurIntensity);
					#endif
					#endif
					...
					//OUTLINE-------------------------------------------------------------
						...
						#if OUTDIST_ON
						fixed2 outDistUv = i.uv;
						outDistUv.x += (_Time * _OutlineDistortTexXSpeed) % 1;
						outDistUv.y += (_Time * _OutlineDistortTexYSpeed) % 1;
						#if ATLAS_ON
						outDistUv = fixed2((outDistUv.x - _MinXUV) / (_MaxXUV - _MinXUV), (outDistUv.y - _MinYUV) / (_MaxYUV - _MinYUV));
						#endif
						fixed outDistortAmnt = tex2D(_OutlineDistortTex, outDistUv).r * 0.1 * _OutlineDistortAmount;
						...
						#if OUTTEX_ON
						fixed2 tempOutDist = i.uv;
						tempOutDist.x += (_Time * _OutlineTexXSpeed) % 1;
						tempOutDist.y += (_Time * _OutlineTexYSpeed) % 1;
						#if ATLAS_ON
						tempOutDist = fixed2((tempOutDist.x - _MinXUV) / (_MaxXUV - _MinXUV), (tempOutDist.y - _MinYUV) / (_MaxYUV - _MinYUV));
						#endif
						fixed4 tempOutColor = tex2D(_OutlineTex, tempOutDist);
						...
						stripes = 1 - step(_HologramStripesFill, rand(fixed2(stripes, stripes)));
